...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, get_encoder_feature=False):
        ...
        for block in self.transformer.resblocks:
            x = block(x)
            all_feats.append(x.permute(1, 0, 2))
...
class VisionTransformer_MaPLe(nn.Module):
    ...
    def forward(self, x: torch.Tensor, shared_ctx, compound_deeper_prompts, get_encoder_feature=False):
        x = self.conv1(x)  # shape = [*, width, grid, grid]
        x = x.reshape(x.shape[0], x.shape[1], -1)  # shape = [*, width, grid ** 2]
        x = x.permute(0, 2, 1)  # shape = [*, grid ** 2, width]
        x = torch.cat(
        ...
        all_feats = []
        outputs = [x, compound_deeper_prompts, 0]
        for block in self.transformer.resblocks:
            outputs = block(outputs)
            all_feats.append(outputs[0].permute(1, 0, 2))
...
class CLIP(nn.Module):
    ...
    def encode_image(self, image, get_encoder_feature=False):
        return self.visual(image.type(self.dtype), get_encoder_feature=get_encoder_feature)
