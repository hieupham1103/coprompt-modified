...
class CustomCLIP(nn.Module):
    def __init__(
        ...
        self.lambd_pca = cfg.TRAINER.CoPrompt.PCA_CONSISTENCY_LOSS
        self.pca_dim = cfg.TRAINER.CoPrompt.PCA_DIM
        self.pca_vectors_num = cfg.TRAINER.CoPrompt.PCA_VECTORS_NUM
        self.num_layers_for_pca = cfg.TRAINER.CoPrompt.PCA_LAYER
        
        
    def PCA_decompose(self, input_tensor: torch.Tensor):
        B, T, D_feat = input_tensor.shape
        input_tensor_float = input_tensor.float()
        mean_feature_vector_per_sample = input_tensor_float.mean(dim=1, keepdim=True)  # Shape: (B, 1, D_feat)
        X_centered = input_tensor_float - mean_feature_vector_per_sample  # Shape: (B, T, D_feat)

        try:
            U, S_values, Vt = torch.linalg.svd(X_centered, full_matrices=False)
        except Exception as e:
            print(f"SVD failed: {e}. Input tensor shape: {X_centered.shape}, dtype: {X_centered.dtype}, device: {X_centered.device}")
            raise e
            
        U_selected = U[:, :self.pca_vectors_num, :self.pca_dim]
        S_selected_values = S_values[:, :self.pca_dim]
        Vt_selected = Vt[:, :self.pca_dim, :self.pca_vectors_num]

        return U_selected, S_selected_values, Vt_selected
    ...
    def forward(self, image1, image2=None, label=None):
        ...
        student_cls_token, student_encoder_features  = self.image_encoder(
            image1.type(self.dtype), shared_ctx, deep_compound_prompts_vision,
            get_encoder_feature=True
        )

        x_a = self.adapter_image(student_cls_token)
        student_cls_token = (
            self.image_adapter_m * x_a + (1 - self.image_adapter_m) * student_cls_token
        )
        ...
        student_cls_token = student_cls_token / student_cls_token.norm(dim=-1, keepdim=True)
        text_features = text_features / text_features.norm(dim=-1, keepdim=True)

        logits = logit_scale * student_cls_token @ text_features.t()

        if self.prompt_learner.training:
            pre_trained_text_features = self.ori_embedding[
                random.randint(0, self.ori_embedding.shape[0] - 1)
            ]
            teacher_cls_token, teacher_encoder_features = self.model_distill.encode_image(
                                    image2,
                                    get_encoder_feature=True
                                )
            ...
            teacher_cls_token = (
                teacher_cls_token
                / teacher_cls_token.norm(dim=-1, keepdim=True)
            )
            ...
            if self.distill_criteria == "cosine":
                ...
                score = cos(student_cls_token, teacher_cls_token)
                ...
                ) + F.mse_loss(student_cls_token, teacher_cls_token)
                
                
            pca_consistency_loss = torch.tensor(0.0).to(image1.device)
            if student_encoder_features and teacher_encoder_features:
                actual_pca_layers = min(self.num_layers_for_pca, len(student_encoder_features), len(teacher_encoder_features))

                for layer_idx in range(actual_pca_layers):
                    maple_feat_layer = student_encoder_features[layer_idx]  # (B, L_student, D)
                    clip_feat_layer = teacher_encoder_features[layer_idx]  # (B, L_teacher, D)

                    teacher_patches = clip_feat_layer[:, :, :]
                    student_patches = maple_feat_layer[:, :, :]

                    try:
                        U_s, _, Vt_s = self.PCA_decompose(student_patches)
                        U_t, _, Vt_t = self.PCA_decompose(teacher_patches)
                    except Exception as e:
                        print(f"PCA Align: Error during PCA for layer {layer_idx}: {e}")
                        continue


                    # --- Align các "U mean" (U_mean_over_tokens) ---
                    cos_sim_loss_fn = nn.CosineSimilarity(dim=1, eps=1e-8)
                    current_align_loss = (1.0 - cos_sim_loss_fn(U_s.mean(dim=1), U_t.mean(dim=1))).mean()

                    # --- Align các "Vt mean" (Vt_t_mean_over_components) ---
                    current_align_loss_vt = (1.0 - cos_sim_loss_fn(Vt_s.mean(dim=1), Vt_t.mean(dim=1))).mean()
                    current_align_loss += current_align_loss_vt # Cộng vào nếu dùng

                    pca_consistency_loss += current_align_loss
                
                if actual_pca_layers > 0 :
                    pca_consistency_loss = pca_consistency_loss / actual_pca_layers
            
            total_loss = loss + self.lambd * loss_distill + self.lambd_pca * pca_consistency_loss
            # import pdb; pdb.set_trace()
            return total_loss, loss, loss_distill, pca_consistency_loss
...
class CoPrompt(TrainerX):
    ...
    def forward_backward(self, batch):
        ...
        if prec == "amp":
            with autocast():
                loss, main_loss, distill_loss, pca_loss = model(image1, image2, label)
            loss = loss.mean()
            main_loss = main_loss.mean()
            distill_loss = distill_loss.mean()
            pca_loss = pca_loss.mean()
            optim.zero_grad()
        ...
        loss_summary = {"loss": loss.item(), "main_loss": main_loss.item(),
                       "distill_loss": distill_loss.item(), "pca_loss": pca_loss.item()}
